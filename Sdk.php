	...
	**/
	public function createLoader($checkpointer, $opts=[]) {
		...
		switch($opts['config']['uploader']) {
			case "firehose":
				$uploader = new lib\Firehose($this->id, $this->config['firehose'], "us-west-2");
				$massuploader = new lib\Mass($this->id, $this->config['s3'],"us-west-2", $uploader);
				break;
			case "kinesis":
				$uploader = new lib\Kinesis($this->id, $this->config['kinesis'],"us-west-2");
				$massuploader = new lib\Mass($this->id, $this->config['s3'],"us-west-2", $uploader);
				break;
			case "mass":
				$kinesis = new lib\Kinesis($this->id, $this->config['kinesis'],"us-west-2");
				$uploader = new lib\Mass($this->id, $this->config['s3'],"us-west-2", $kinesis);
				break;
		}
